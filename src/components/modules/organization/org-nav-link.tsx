...
const OrgNavItem = ({
  ...
  title: "Outstations" | "Societies" | "Communities";
  to: string;
  className?: string;
}) => {
  ...
  return (
    <>
      ...
      >
        <NavItem
          href={to}
          defaultClasses="curser-pointer hover:text-primary-700 pt-5 pb-4"
      ...
      <nav className="flex flex-col gap-2 ml-4 pl-4 border-gray-400 border-l-2">
        {showSubMenu &&
          subNav.map((na, indx) => {
            return (
              <Link
                className={`py-2 border-gray-400 border-b-1 text-sm ${
                  indx === subNav.length - 1 ? "border-none" : ""
                }`}
                key={indx}
                href={`${title}/${na.slug}`}
              >
